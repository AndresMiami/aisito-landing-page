...
const config = {
  ...
  // --- Google Maps Autocomplete (Keep from your original, ensure IDs match HTML) ---
  function initAutocomplete() {
      // Check if google.maps is loaded
      if (typeof google === 'undefined' || typeof google.maps === 'undefined' || typeof google.maps.places === 'undefined') {
          console.error("Google Maps API not loaded correctly. Autocomplete cannot initialize.");
          // Maybe show an error to the user or disable address fields?
          return;
      }
      ...
      if (fromLocationInput) {
          try {
              ...
              acFrom.addListener('place_changed', () => {
                  ...
                  updateAirportFieldVisibility('from-location', place?.types?.includes('airport') || false);
              });
      ...
      if (toAddressInput) {
          try {
              ...
              acTo.addListener('place_changed', () => {
                  ...
                  updateAirportFieldVisibility('to-address', place?.types?.includes('airport') || false);
              });
  ...
  function processFormData(elements) {
      ...
      if (activePanelId === '#panel-oneway') {
        ...
        dataObject['to-address'] = formData.get('to-address')?.trim(); // Use correct ID
        dataObject['pickup-date'] = formData.get('pickup-date-oneway');
        dataObject['pickup-time'] = formData.get('pickup-time-oneway');
        dataObject.vehicle_type = formData.get('vehicle_type_oneway'); // Get from radio name
      } else if (activePanelId === '#panel-experience-plus') {
  ...
  function initializeEventListeners(elements, placeholders, config) {
       ...
       elements.bookingForm?.addEventListener('submit', (event) => {
           ...
           if (validateForm(elements)) {
               ...
               if (activePanelId === '#panel-oneway') {
                   ...
                   if (data.service_type === 'Hourly') {
                       // Hourly also gets a quote first
                       submitQuoteRequest(data);
                   } else {
                       // Curated Experience - Submit directly (or add quote step later)
    ...
    // --- Main Execution ---
    // Get element references and configuration
    const elementRefs = getElementRefs();
    const appConfig = config;
  
    // Defer initialization requiring Google Maps API until the callback runs
    // The Google Maps callback 'initAutocomplete' will trigger the rest of the setup.
